...
import { Upload, FolderOpen, Album, Clock, CheckCircle, Loader2, XCircle, Trash2, Sparkles, ScanFace, X } from "lucide-react";
import type { RecentUpload } from "@/types";

const STATUS_MAP: Record<string, { label: string; icon: typeof Clock; color: string }> = {
  pending: { label: "等待中", icon: Clock, color: "text-amber-500" },
  processing: { label: "AI 处理中", icon: Loader2, color: "text-blue-500" },
...
export function UploadPage() {
  ...
  // Compute processing outcome counts from tracked uploaded images
  const uploadedIds = result ? new Set(result.image_ids) : null;
  const trackedUploads = uploadedIds ? uploads.filter((u) => uploadedIds.has(u.id)) : [];
  const processingDone = result && trackedUploads.length === result.image_ids.length &&
    trackedUploads.every((u) => u.processing_status === "done" || u.processing_status === "error");
  const successCount = trackedUploads.filter((u) => u.processing_status === "done").length;
  const failCount = trackedUploads.filter((u) => u.processing_status === "error").length;

  // Auto-dismiss banner 10s after processing completes
  useEffect(() => {
    if (!processingDone || dismissing) return;
    const timer = setTimeout(() => setDismissing(true), 10000);
    return () => clearTimeout(timer);
  }, [processingDone, dismissing]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {result && (
        <div
          onTransitionEnd={() => { if (dismissing) { setResult(null); setDismissing(false); } }}
          className={`p-4 rounded-lg border transition-all duration-500 relative ${
            processingDone && failCount > 0 ? "bg-amber-50 border-amber-200" : "bg-green-50 border-green-200"
          } ${
            dismissing ? "opacity-0 max-h-0 py-0 border-0 mb-0 overflow-hidden" : "opacity-100 max-h-24"
          }`}
        >
          <div className="flex items-center justify-between">
            <div>
              <p className={`font-medium ${processingDone && failCount > 0 ? "text-amber-700" : "text-green-700"}`}>
                上传完成
              </p>
              <p className={`text-sm ${processingDone && failCount > 0 ? "text-amber-600" : "text-green-600"}`}>
                {processingDone
                  ? `处理完成：${successCount} 张成功${failCount > 0 ? `，${failCount} 张失败` : ""}`
                  : `共上传 ${result.uploaded} 张图片，AI 正在后台处理...`}
              </p>
            </div>
            <button
                    ...
                    <tr key={img.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2">
                        <div className="flex items-center gap-1.5">
                          <StatusIcon size={14} className={`${st.color} ${(img.processing_status === "processing" || img.processing_status === "captioning" || img.processing_status === "faces") ? "animate-spin" : ""}`} />
                          <span className={st.color}>{st.label}</span>
                        </div>
                        {img.tasks && img.tasks.length > 0 && img.processing_status !== "done" && (
                            ...
                            {img.tasks.map((t) => (
                              <span
                                ...
                                className={`text-xs px-1.5 py-0.5 rounded ${
                                  ...
                                  t.status === "error" || t.status === "timeout" ? "bg-red-100 text-red-700" :
